...
def get_melone_chart():
    ...
    for tr in trs:
        ...
        href = tr.select_one('.ellipsis.rank01 > span > a')['href'][-10:-2]
        art_href = tr.select_one('.ellipsis.rank02 > a')['href']
        art_href = re.sub('[^0-9]', '', art_href)
        data.append({'순위':rank, '제목':title, '아티스트':artist, '앨범':album,
                    '이미지':img, 'href':'https://www.melon.com/song/detail.htm?songId='+href,
                    '가수정보':art_href})
